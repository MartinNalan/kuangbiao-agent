...
Use the evidence endpoint when another agent or customer system will organize the final language:
...
The response has no `answer` field. Check `answerable=true` and `status=ready`, then use only
the returned `sources` to synthesize a conclusion. Each source can include stable document,
unit, and retrieval IDs; standard number; clause; page range; short quotation; retrieval routes;
and governance status. `missing_evidence` identifies unresolved evidence targets. The private
`/knowledge/*` service remains inaccessible to the caller.
