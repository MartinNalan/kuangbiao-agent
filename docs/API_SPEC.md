...
This authenticated endpoint is the public evidence boundary for downstream agents. It uses the same T085 question understanding and prepared retrieval plan, hybrid retrieval, evidence reranking, authority guards, and sufficiency checks as `/api/ask`, then stops before final answer synthesis. It never exposes private `/knowledge/*` routes or unrestricted corpus content.
...
`status` is `ready`, `insufficient_evidence`, `out_of_scope`, or `clarification_required`. A downstream model may synthesize a conclusion only when `status=ready` and `answerable=true`; every factual statement must remain traceable to `sources`. The endpoint has no `answer` field and does not invoke the final answer model. `missing_evidence` lists unresolved required evidence targets.

A ready or in-scope insufficient-evidence result consumes one basic quota unit. Clarification and out-of-scope results do not consume quota. A clarification response must be shown to the caller or replaced by a new, complete question; the current evidence request contract does not accept a stored clarification selection.
