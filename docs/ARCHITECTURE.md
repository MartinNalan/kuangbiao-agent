...
生产状态以 `coordination/task_board.md` 的 T083 为准。T085 的 Schema 前移和统一问题理解/检索规划已经进入本地代码，但只有完成单独部署验收后才能视为线上状态。
...
T085 后的本地代码把固定输出 Schema 放在动态问题之前，以提高模型前缀缓存稳定性。QuestionResolver 默认同时生成问题理解和检索规划；当确定性完整性校验发现多组 AND 证据却没有独立检索变体时，系统才回退到专用 RetrievalPlanner。模型不可删除用户明确给出的标准号、文号、数值、矿种、业务事项和限定条件。
