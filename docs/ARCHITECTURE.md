...
![geowiki v3.1.0 系统架构与用户问答处理流程](SYSTEM_ARCHITECTURE_DIAGRAM.svg)

该图对应当前 v3.1.0 的实际部署和数据边界：公网只访问 Nginx 与应用 API；私有知识库、标准正文、向量索引与知识图谱不对公网开放。SVG 可直接在浏览器、GitHub 或支持图片预览的文件管理器中打开。
...
- Query plan：本地确定性规则统一 `1/I/Ⅰ/一类型` 等无歧义写法；每个通过领域门控的问题均先由 DeepSeek 校准 `采矿正/采矿证` 等领域输入并提取业务字段。模型规划器随后生成规范化问题、证据目标和最多 3 条本地知识库子查询；受保护问题保留用户明确的标准号、文号、数值、事项类型和输出模式，模型不能用自由改写覆盖系统 Schema。
...
- Evidence-target retrieval：模型拆出的复合关系在首轮并行检索。子查询可放开系统推定的文件锚点，但只在模型指定的文件类型内检索；用户明确的标准号或筛选条件始终保留。
- Evidence judge：复杂问题或存在多个证据目标时，由 DeepSeek 审查前 10 个候选是否分别覆盖各关系；最终答案只能依据审查保留的条款生成。
