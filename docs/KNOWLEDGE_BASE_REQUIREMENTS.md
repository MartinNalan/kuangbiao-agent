...
知识库服务用于支撑 geowiki 的专业问答。它不是简单的文件仓库，而是一个能够返回可引用证据的检索服务。
...
知识库本体永远不作为公开产品暴露。原始文件、OCR 全文、chunk 全文、索引、数据库、目录内部字段和 `/knowledge/*` 服务都属于后台维护的核心私有资产。对外仅开放受控问答 API 的结果，不开放知识库下载、镜像、全文库、索引或直接检索入口。
...
Knowledge API
  -> /knowledge/search
  -> /knowledge/standards
  -> /knowledge/intent/normalize

Domain Lexicon
  -> user expression, canonical term, intent label, expansion rules
```
...
- `DomainLexicon`: 领域术语、口语表达、意图标签和检索扩展规则。
...
- `DomainLexicon` normalizes user expression to canonical term and intent label
...
知识库应维护可版本化的 `domain_lexicon`，用于把用户自然语言中的口语表达、简称、旧称和业务说法归一为标准术语与检索意图。它不是普通同义词表，而是检索前置层的一部分。
...
- `user_expression`: 用户可能输入的表达，例如“采矿证”“储量报告评审”“沙金”“工程距离”。
- `canonical_term`: 规范表达，例如“采矿许可证”“矿产资源储量评审备案”“砂金”“勘查工程间距”。
- `intent_label`: 归一后的意图，例如 `authority_responsibility`、`standard_selection`、`numeric_table_lookup`、`clause_comparison`、`status_verification`、`definition_explanation`、`out_of_scope`。
- `domain`: 适用领域，例如 `solid_mineral`、`policy_authority`、`exploration_standard`、`reserve_estimation`。
- `positive_expansions`: 检索时应主动扩展的词，例如“哪个机构”扩展为“谁负责、负责部门、权限归属、自然资源主管部门负责”。
- `negative_terms`: 需要降权或排除的词，例如金矿储量评审机构问题中，油气、煤层气类证据应降权。
- `evidence_required_patterns`: 证据必须满足的模式，例如权限类问题必须包含“负责”“备案”“自然资源部”“省级自然资源主管部门”等负责主体或事项。
- `priority`: 高价值规则优先级。权限归属、标准适用、表格数值和条款差异类规则应高于普通关键词扩展。
- `status`: `active`、`draft`、`disabled`，用于灰度测试和回滚。

第一阶段必须覆盖以下高价值词条：

| 用户表达 | 规范表达 | 意图 | 备注 |
| --- | --- | --- | --- |
| 采矿证 | 采矿许可证 | `authority_responsibility` | 用于储量评审备案权限判断 |
| 探矿证 | 勘查许可证 | `authority_responsibility` | 用于储量评审备案权限判断 |
| 储量报告评审 | 矿产资源储量评审备案 | `authority_responsibility` | 不应按矿种或矿山规模决定机构 |
| 去哪个机构、找谁办、谁负责 | 负责部门、权限归属 | `authority_responsibility` | 必须召回负责主体条款 |
| 大型金矿、小型矿山 | 矿种或规模描述 | `authority_responsibility` | 对评审机构问题只能作为背景，不作为权限依据 |
| 沙金 | 砂金、金属砂矿类 | `standard_selection` | 用于推荐适用标准 |
| 工程距离 | 勘查工程间距 | `numeric_table_lookup` | 必须命中表格或结构化数值 |
| 外推、矿体外推 | 资源量类型与矿体外推 | `clause_comparison` | 必须能比较不同条款差异 |

知识库可以先通过静态配置或数据库表实现 `domain_lexicon`，但必须预留后续由管理员维护、导入和版本化的能力。
...
- 检索前先调用 `domain_lexicon` 做术语归一、意图识别和查询扩展。
- 本地规则应统一阿拉伯数字、罗马数字和中文数字等常见写法，例如将 `1类型`、`I类型`、`Ⅰ类型`、`一类型` 归一为同一检索条件。
- 标准号、条号、术语优先走全文搜索。
...
- 答案生成前执行证据可回答性校验，过滤与意图不一致的证据；如果过滤后无有效证据，应返回证据不足或触发后台补库。
...
- `authority_responsibility`: 优先召回政策、制度文件和知识图谱中 `Organization -> RESPONSIBLE_FOR -> Responsibility` 类关系；证据必须包含负责主体和负责事项。
- `standard_selection`: 优先召回标准目录、标准标题、适用范围和矿种关系；不能只因为文本中出现矿种就判断适用。
- `numeric_table_lookup`: 优先召回表格、结构化单元格和条款级数值；答案必须保留行列含义，例如走向、倾向、坑探、钻探。
- `clause_comparison`: 优先召回多个标准的同类条款；答案必须直接列出差异点，不应只提示用户自行对照原文。
- `status_verification`: 优先使用官方标准目录、替代关系和状态来源字段。
- `out_of_scope`: 不进入大模型自由问答，不记录为补库任务。

可回答性校验规则：

- 权限归属类问题不能用矿山规模、矿种或技术规范替代行政权限证据。
- 固体矿产问题应降权油气、煤层气等不适用证据，除非用户明确询问这些领域。
- 表格数值类问题如果只召回普通段落、缺少行列定位，应返回“不足以准确读取表格值”或触发后台结构化补库。
- 条款差异类问题如果只能召回单一文件，应明确说明只找到一个来源，不能声称存在或不存在不一致。
- 官方来源 URL 为空时，仍可内部回答，但应在质量标记中提示来源链接待补全。
...
- `STANDARD_ISSUED_BY`
- `STANDARD_HAS_STATUS`
- `ORGANIZATION_RESPONSIBLE_FOR`
- `DOCUMENT_DEFINES_AUTHORITY`
- `STANDARD_APPLIES_TO_TASK`

第一阶段优先抽取高价值关系：

- `Organization -> RESPONSIBLE_FOR -> Responsibility`：用于回答“谁负责、哪个机构、哪一级部门办理”。
- `Standard/Policy -> DEFINES_AUTHORITY -> Responsibility`：用于把行政权限结论追溯到具体政策条款。
- `Standard -> APPLIES_TO -> Mineral/Task`：用于回答“某矿种、某事项应使用哪个标准”。
- `Clause -> SPECIFIES_NUMERIC_RULE -> Requirement`：用于工程间距、外推距离、规模划分等数值型问题。
- `Clause -> DIFFERS_FROM -> Clause`：用于沉淀人工确认过的条款差异，例如不同标准对矿体外推距离依据的不同表述。
...
- 云端受控 API 不开放知识库本体、全文库、索引、数据库或直接检索入口；默认仅返回必要、受控、限长的证据片段和来源定位。
