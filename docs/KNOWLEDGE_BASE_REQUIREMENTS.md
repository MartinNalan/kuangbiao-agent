...
- MVP 阶段先入库 10-30 个高频核心标准，用于验证检索、引用、OCR 和问答链路。
...
如果项目初期希望更轻量，可以先用 SQLite/PostgreSQL + Elasticsearch；对象存储和任务队列可用本地目录与简单后台进程替代。
...
- `review_pending`: 用户申请进入公共知识库。
- `approved_public`: 管理员审核通过，进入公共知识库。
- `rejected`: 审核拒绝。
...
未审核或审核未通过的用户上传资料不能影响其他用户，也不能作为公共答案依据。
