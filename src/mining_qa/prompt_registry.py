...
PROMPT_REGISTRY_VERSION = "v3.2.0"
...
_INTENT_RULES = {
    ...
    "reserve_estimation_basis": (
        "储量技术依据问题必须先引用现行规范中资源量转换为储量的最低技术经济评价要求，再按勘查、"
        "矿山建设、正常生产和长期停产阶段列明适用依据。必须把规范性原文与《300问》等解读材料"
        "分层标注；不得把评审备案申请函、信息表等行政报件混入技术依据，也不得反向使用采矿权"
        "申请材料清单证明储量评审备案自身的材料要求。"
    ),
    "technical_method": (
        "技术问题必须区分等级满足判断与试验符合性核验。等级满足判断默认用户所述试验属于该等级，"
        "只按同一技术体系且有规范原文支持的层级关系比较；不得追加样品、设备、时长或流程核验条件。"
        "只有用户明确询问试验是否合格、样品、设备、时长、采样或记录时，才进入符合性核验。"
        "对于“勘查阶段 + 技术性能要求”问题，条件未完整时应先输出标准的完整条件矩阵，"
        "不能强制用户先确认矿石类型。选冶试验程度判断必须同时核对适用矿种勘查规范和"
        "DZ/T 0340-2020：前者确定矿种、阶段要求，后者核对试验分类和层级；"
        "两者不一致时应显示冲突，不得自行消解。"
    ),
}
